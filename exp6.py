...
def run(rank, flags):
    ...
    train_dict['batch_lr_schedule'] = torch.optim.lr_scheduler.CosineAnnealingLR(train_dict['optimizer'], len(train_dict['train_loader'])*flags['epochs'])
    train_dict['epoch_lr_schedule'] = None
...
# model
model_fn = torchvision.models.wide_resnet50_2
if FLAGS['continue_from']:
    net = model_fn(pretrained=False)
